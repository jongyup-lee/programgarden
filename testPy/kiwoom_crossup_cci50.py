...
class Kiwoom(QAxWidget):
        ...
        if sRQName == "예수금상세현황요청":
            ...
            if sPrevNext == "2":
                ...
                #    moving_average_price = total_price / 120 # 조회한 당일 기준 120일간의 평균가격 (기준일을 변경하지 않는 한 루프가 끝나는 순간까지 변하지 않음)
                    # 1번, 평균거래량을 구하기 위한 거래량 축적 리스트
                    for value in self.calcul_data[:120]:
                        self.gValueList.append(value)
                    print("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% %s" % self.conditional_search01(self.gValueList))
    ...
    def realdata_slot(self, sCode, sRealType, sRealData):
        ...
        if sRealType == "장시작시간":
            ...
            ### 매수를 위한 조건
            ############################################################################################################
            ## 요소 1 : 틱 단위 평균가격 (고가 + 저가 + 현재가) / 3
            ##      1.1 - 평균가격의 기준은 무엇인가
            ## 요소 2 : 평균 편차 = |평균 가격 - n일 이동평균|
            ## 요소 3 : 평균 절대 편차 = 각 거래일의 평균 편차의 합 / 총 거래일 수
            ## CCI = (평균 가격 - n일 이동평균) / (0.015 * 평균 절대 편차)

            ## 조건 1 : CCI값이 0보다 커야 함
            ## 조건 2 : 시작가가 20일 이동 평균 값 보다 커야 함

            ## 추가 조건 1 : 시작가가 20일 이동 평균 값보다 작고 현재가가 20일 보다 큰 경우
            ## 추가 조건 2 :
            ############################################################################################################

            data = pd.read_excel("cci.xlsx", index_col=0, engine='openpyxl')
            cci = self.getCCI(data, 50)
    ...
    ###CCI구하는 함수
    def getCCI(data, ndays=50):
        tp = (data['hight'] + data['low'] + data['close']) / 3
    ...
    # 입력데이터 : 종가(list)
    # 리턴데이터 : boolean
    def conditional_1search02(self, data):
        tmp_stock = []
        rtn_flag = False
        if len(data) > 100:
    ...
    # 입력데이터 : 종가(int)
    # 리턴데이터 : boolean
    def conditional_1search03(self, data):
        rtn_flag = False
        if 1000 <= data <= 50000:
            rtn_flag = True
    ...
    def conditional_1search04(self, data):
        rtn_flag = False
        if 100 <= data >= 1000:
            rtn_flag = True
    ...
    def conditional_1search05(self, data):
        rtn_flag = False

        if len(data) > 2:
            one_candle_ago = data[-2]  # 1봉 전의 종가
            zero_candle_ago = data[-1]  # 0봉 전의 종가
    ...
    def conditional_1search06(self, data):
        rtn_flag = False
    ...
    def conditional_1search07(self):
        pass

    # 8. 유보율 : 최근결산 500% 이상
    def conditional_1search08(self):
        pass
