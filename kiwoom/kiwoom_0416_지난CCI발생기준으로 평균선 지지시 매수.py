...
class Kiwoom(QAxWidget):
    ...
    def realdata_slot(self, sCode, sRealType, sRealData):
        ...
        if sRealType == "장시작시간":
            ...
            rslt_pastSignals = self.getCCI(i, k, b, sCode, 50)
            ...
            ma20_price = self.getMaPrice(i, k, b, sCode, 20)
            ma20_ratio = ((b - int(ma20_price)) / int(ma20_price))

            increased_price = b + (b * ma20_ratio)

            print('-----------------------------------------------------------------------------------------------------------------------------')
            print('[ANALYZE] (현재시간 : %s ) 종목명 : %s(%s)' % (datetime.now(), code_nm, sCode))
            print('[ANALYZE] CCI 신호검색 결과 => %s' % self.getToDaySignal(rslt_pastSignals))
            print('[ANALYZE] 현재가(%s) >  20평균가(%s)  | 체결강도 : 0 < %s < 1000' % (b, ma20_price, o))
            ...
            self.logger.info('[ANALYZE] CCI 신호검색 결과 => %s' % self.getToDaySignal(rslt_pastSignals))
            self.logger.info('[ANALYZE] 현재가(%s) >  20평균가(%s)  | 체결강도 : 0 < %s < 1000' % (b, ma20_price, o))
            ...
            if sCode in self.account_stock_dict.keys() and sCode not in self.jango_dict.keys():
                ...
                print("[DEBUG] 보유주")
                self.logger.info("[DEBUG] 보유주")
                if asd['매매가능수량'] > 0:
                    if b >= increased_price:

                        print('[보유주 - 익절매도조건] 현재가(%s) >= 상승비율가(%s))' % (b, increased_price))
                        self.logger.info('[보유주 - 익절매도조건] 현재가(%s) >= 상승비율가(%s))' % (b, increased_price))
                        self.logger.info("[보유주 - 익절매도] %s(%s) | 매도 수량 : %s | 체결단가 : %s | 매입가 : %s | 20평균 : %s | 상승비율가 : %s"
                        ...
                        if sCode in self.addBuyHistory:
                            ...
                            self.clearAndSaveExcel(self.addBuyHistory, 'addBuyHistory')
            ...
            # 1. 계좌 잔고 평가 내역에 없고 당일 실시간 잔고에 있음
                ...
                if jd['주문가능수량'] > 0:
                    ...
                    if order_success == 0:
                        ...
                        self.logger.info("[info] 매도주문 전달 실패")
                        #print("[info] 매도주문 전달 실패")
            elif b > ma20_price and 100 < o < 1000 and self.sumCrntPrce1[sCode]['crntprce'] < self.sumCrntPrce2[sCode]['crntprce'] and self.sumCrntPrce2[sCode]['crntprce'] < self.sumCrntPrce3[sCode]['crntprce']:
                print('[매수조건] 20평균보다 위 / 체결강도 100이상 / 누적주가 부합')
                if e != 0:
                ...
                if quantity > 0:
                    ...
                    if 0.5 <= ma20_ratio * 100 < 2:
                        print("[매수조건] 0.5 <= 이격률(%s) < 2 |" % (ma20_ratio * 100))
                        self.logger.info("[매수조건] 0.5 <= 이격률(%s) < 2 |" % (ma20_ratio * 100))

                        if sCode in self.buyHistory or sCode in self.addBuyHistory:
                            pass
                        elif self.getToDaySignal(rslt_pastSignals) == True:
                            print('[매수조건] 당일 CCI 신호 발생')
                        ...
                        elif self.getToDaySignal(rslt_pastSignals) == False and rslt_pastSignals["Date"].iloc[-1]:
                            print('[매수조건] 당일 CCI 신호 발생')
                       ...
                       self.logger.info("[info] 이격도 조건 불합격 : 0.5 < %s < 2" % ma20_ratio)
                       print("[info]  이격도 조건 불합격 : 0.5 < %s < 2" % ma20_ratio)
    ...
    def chejan_slot(self, sGubun, nItemCnt, sFidLit):
        ...
        if intGubun == 0:
            ...
            #   1. StockName, Ratio, 20MA, BBUpper, std
            #       1) 해당 항목들을 연산하려면 시간이 오래걸릴수 있으니
            #       2) SKIP하는 방안을 고민해봐야 함
    ...
    def read_code02(self):
        ...
        if os.path.isfile(os.path.join("historyXlsx", 'buyHistory.xlsx')):
            ...
            if bh is not None:
                ...
                print('buyHistory : %s' % self.buyHistory)
    ...
    def getMaPrice(self, highVal, lowVal, closeVal, sCode, ndays=20):
    ...
    def getToDaySignal(self, data):
        rtnVal = False

        if len(data) > 0 and data["Date"].iloc[-1] == datetime.today().date():
            rtnVal = True
        else:
            pass
