    ...
    def realdata_slot(self, sCode, sRealType, sRealData):
        if sRealType == "장시작시간":
            ...
            # 1. 계좌 잔고 평가 내역에 없고 당일 실시간 잔고에 있음
                ...
                cciDF = pd.read_excel('xlsx/' + sCode + '.' + code_nm + '.xlsx', engine='openpyxl')
                # 실시간 현재가로 마지막 'Close' 값을 업데이트
                cciDF.at[cciDF.index[-1], 'Close'] = b
                self.tmpStockDict.update({sCode: {}})
                self.tmpStockDict[sCode].update(
                    {'Date': cciDF['Date'].to_list(), 'High': cciDF['High'].to_list(), 'Low': cciDF['Low'].to_list(),
                     'Close': cciDF['Close'].to_list()})

                crossup_signals = self.getCCI(sCode)

                if quantity > 0:
                    print("[매수조건] 매수가능수량 : %s" % quantity)
                    if sCode in self.buyHistory or sCode in self.addBuyHistory:
                        pass
                    elif self.getTodaySignalCheck(crossup_signals) == 'Today' and b > ma20_price:
                        print("[매수조건] CCI시그널 : %s / 20일 이동평균 : %s / 현재가 : %s" % (self.getTodaySignalCheck(crossup_signals), ma20_price, b))
                        self.logger.info("[매수조건] CCI시그널 : %s / 20일 이동평균 : %s / 현재가 : %s" % (self.getTodaySignalCheck(crossup_signals), ma20_price, b))
