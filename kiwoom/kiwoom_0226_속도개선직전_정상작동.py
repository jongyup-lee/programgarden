...
#
                ...
                if self.gPass_success == True:
                    ...
                    self.logger.info('조건부 통과됨 : %s (%s)' % (code_nm, code))
                    print("조건부 통과됨")
                    ########################################################################################################
                    # 해당 종목의 모든 거래일 데이터를 받아와서 self.calcul_data에 저장
                    ########################################################################################################
    ...
    def realdata_slot(self, sCode, sRealType, sRealData):
        ...
        if sRealType == "장시작시간":
            ...
            if sCode in self.portfolio_stock_dict:
                ...
                #condition01 = self.upTrendCheck(sCode, 0, 2, 4)
            ...
            #   고가 대비 현재가의 높낮이를 비교한다
            # if b < i:
            #   담아 놓은 최고가와 현재가와의 등락률을 비교하여 담아 놓은 최고가 대비 현재가가 90% 미만일 경우 판매한다.
            #    bop_rate = (b - i) / i * 100
            #       if bop_rate < -1:
            #       판매
            ...
            if sCode in self.account_stock_dict.keys() and sCode not in self.jango_dict.keys():
                ...
                print('올True')
                if (m * 100000000) > 100000000000 and g > 1000000:
    ...
    def read_code02(self):
        ...
        for filename in os.listdir("xlsx"):
            if os.path.isfile(os.path.join("xlsx", filename)):
                ...
                self.portfolio_stock_dict.update({filecode: {"종목명": stock_nm}})
